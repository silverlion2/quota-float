...
- **Codex 扩展与伴侣：** 控制中心 → 扩展，可只读查看用户级 MCP、插件配置及目录中发现的 Skills，按类型/名称筛选；部分来源不可读时明确提示。内置六个经过核实的 GitHub 伴侣入口，星数标注采集日期；此功能不安装、启用或运行第三方扩展。

- **窗口按需收缩：** 只有一个监控平台时自动收起重复的平台列表；多个平台可手动收起并保留快捷切换。打开设置或洞察时恢复完整宽度，关闭后缩回，同次运行的悬停展开保留选择。
...
- Codex Profile 账户统计：终生 Token、单日峰值、连续使用天数、最长单次任务和任务数读取桌面 Profile 同一接口，不受本机图表筛选影响。本机区间统计单独标注，并补充归档会话明细。
- “重置预测与消耗计划”入口在无数据、账号过期和来源冲突时仍可查看；短暂失败保留有效信号并显示提示，支持单独刷新公开来源。
...
- Vibe Usage 洞察页会增量索引本机仍保留的全部 Codex 会话元数据，支持模型/项目/终端筛选、默认 24 小时或完整历史趋势、自定义本地日期区间（最多 366 天，包含结束日）、会话与活跃度指标、分时热力图和分模型 API 等价费用预估。自定义区间同步作用于额度历史与用量导出。
...
- Windows 新增任务栏模式：在控制中心选择“任务栏”，即可在系统通知区域显示当前平台的剩余额度数字，点击打开详情，收起后不占桌面。如果 Windows 将图标收进隐藏区域，可手动拖到任务栏。见[实现与验收说明](docs/TASKBAR-2026-09-22.md)。
- 保留 Float/Ring/Bar/Bottleneck 四种桌面紧凑布局与 Dashboard/Cockpit/Provider bar/Stacked 四种展开布局；瓶颈栏按风险排列每个平台最紧张的额度周期。驾驶舱把额度环、近期趋势、节奏计划与 90 天热力图聚合到一屏，每个区块都可原位放大或拆成独立置顶窗口。
...
- 磁吸 Bar 按实际平台数量收缩：单平台顶部 `224×38`、侧栏 `64×156`，每新增平台才增加一格；Bottleneck 单平台为 `196×38` / `64×100`。原生窗口另含每边 4px 透明边距，保持正向文字、向屏幕内展开和沿边位置。
