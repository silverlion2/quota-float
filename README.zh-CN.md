...
Quota Float 只会把各平台已有令牌发送到该平台自己的额度服务；火山方舟访问保持在 Ark CLI 内，Antigravity 通过本机回环额度服务读取。应用仅保存自身偏好、有限数量的额度采样、事件摘要、布局方案和恢复点。
...
不是。Quota Float 展示平台返回的额度数据或受支持的本地账号缓存，不会根据提示词或本地 Token 数推算。
...
维护者可从[项目简介](docs/PROJECT-SUMMARY.md)、[架构说明](docs/ARCHITECTURE.md)、[路线图](docs/ROADMAP.md)和[桌面开发 SOP](docs/DESKTOP-DEVELOPMENT-SOP.md)开始。
