...
- **Windows taskbar mode:** choose Taskbar in the control center to show the active provider's remaining quota in the native notification area. Click for details, then collapse back to the taskbar. If Windows hides the icon, drag it out of the overflow area. Existing Float/Ring/Bar/Bottleneck modes remain available. See the [implementation and validation notes](docs/TASKBAR-2026-09-22.md).
...
- A Vibe Usage dashboard with incremental indexing across every retained local Codex session metadata file, model/project/terminal filters, session and activity metrics, 24-hour-by-default or complete-history trends, custom local date ranges (up to 366 days, including the end date), an hourly heatmap, and per-model API-equivalent cost estimates. Custom dates also apply to quota history and usage exports.
- A configurable API-equivalent monthly outlook extrapolated from the selected range, shown separately from retained month-to-date cost. Its local alert is checked only while Codex Insights is open. On-demand local week/month reports apply current dimension filters and distinguish recorded hours from known scan truncation. Same-name projects retain separate opaque identities; CSV/JSON exports use anonymous aliases and exclude those identities. An SVG share card and an exportable versioned price catalog are also available.
- Custom accent colors, hidden or condensed providers, reusable layout profiles, and system-login autostart.
- Adaptive expanded windows: a single tracked provider automatically reclaims the provider-list space. Collapse a multi-provider list while keeping quick switching; settings and Insights temporarily restore full width, and your list choice survives hover collapse within the running app.
...
The source version is declared in `package.json` and checked against the native package metadata. Current validation and publication evidence is recorded in the [v0.3.17 release record](docs/RELEASE-0.3.17.md). Source support and automated tests do not imply manual validation on every platform.
