...
- Token 洞察目前只支持 Codex 仍保留文件中的数值型 `token_count` 元数据；其他平台、未知模型、已被 Codex 删除的会话文件，以及确实触发扫描上限而跳过的记录会显示不可用或部分覆盖。项目按 Rust 内规范化路径生成的不透明 ID 区分，basename 仅用于显示，原始路径不进入索引或前端；符号链接等路径别名可能仍被视为不同项目。旧索引会从保留的源文件重建。终端来源只归入有限的规范类别。
