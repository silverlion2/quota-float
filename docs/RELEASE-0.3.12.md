...
The next CI run exposed a size mismatch (dialog bottom 444px against a 268px viewport), initially suspected to be a measurement race. A two-second bounded wait still failed in [CI 34680024024](https://github.com/silverlion2/quota-float/actions/runs/34680024024), and its archived screenshot confirmed persistent clipping. The subsequent audit identified a real application defect: pointer re-entry on an already expanded card called `expand_widget` again, resetting native height to the baseline while the DOM ResizeObserver saw no intrinsic-size change. A follow-up application patch is required; v0.3.12's tag and assets remain unchanged. See [the session record](SESSION-2026-09-12.md) for the correction and validation. CI archives native screenshots for seven days on both success and failure.
