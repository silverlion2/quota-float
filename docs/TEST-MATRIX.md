...
v0.3.22 已公开：423 项本地前端/流程测试、123 项 Windows Rust 测试、112 项 macOS CI Rust 测试、13 项 Windows 原生测试通过；平台跳过、发布产物校验和手工实测限制见 [发布验收](RELEASE-0.3.22.md)。

2026-09-23 详情展开专项：新增原生逐帧采样覆盖 Top/Left/Right 各三次详情展开、洞察高度稳定性，以及收起后迟到 resize 的拒绝；前端覆盖原生完成前不测量、工作区钳制与快速开关的请求身份。证据与本轮验证结果见 [详情展开与可读性](DETAILS-UX-2026-09-23.md)。

2026-09-17 本地待发布：重置预测失败保留、账号状态隔离、四布局/洞察入口、Profile 同源账户累计、归档明细去重及移动幂等。实际最终门禁结果记录在 [预测可见性](RESET-VISIBILITY-2026-09-17.md) 和 [Profile 对齐](CODEX-PROFILE-2026-09-17.md)。

v0.3.16 已公开：334 项本地前端、109 项 exact-source Windows Rust、98 项 macOS Rust、11 项 Windows 原生测试；发布门禁、安装/升级/回滚/卸载、六项资产摘要和双平台更新签名全部通过，详见 [发布验收](RELEASE-0.3.16.md)。

2026-09-15 重置预测：覆盖新响应配旧重置历史、两源佐证后排除、未佐证冲突、缺失基线、同基线分差、来源去重与时效；个人消耗计划覆盖 6 小时窗口、周期匹配、重复时间戳、补充额度、间断/过期和零消耗。实现与精度限制见 [任务记录](RESET-PREDICTION-2026-09-15.md)。

2026-09-15 Codex 扩展中心：新增本地清单筛选、配置/发现状态、部分来源、失败保留、并发刷新保护、原生桥接和精确外链权限覆盖；实际验证和发布结果见 [扩展更新归档](EXTENSIONS-2026-09-15.md)。

v0.3.15 完成 318 项前端、102 项 Windows Rust 和 10 项 Windows 原生测试；源码/发布 CI 成功，六项资产 SHA-256 和双平台更新签名独立通过，草稿候选安装、升级、回滚、重装、卸载通过。平台条件跳过与真实硬件未测范围见 [发布验收](RELEASE-0.3.15.md)。

2026-09-14 自适应布局：新增单平台四布局收缩、多平台折叠导航、弹层恢复、hover 会话选择和宽度/DPI/磁吸回归；完整源码研究、实施及实际验收见 [更新记录](ADAPTIVE-LAYOUT-2026-09-14.md)。
...
最终补充：6 项 Windows 原生 E2E、17 项发布辅助测试通过。v0.3.11 正式产物通过 Defender、安装升级/回滚/重装/卸载检查；下载后核对全部 6 个资产摘要及双平台更新签名，见 [RELEASE-0.3.11.md](RELEASE-0.3.11.md)。
...
| Bar UI | Top 水平条 | 随 1–7 个平台从 `224×38` 增至 `392×38`；Bottleneck 从 `196×38` 增至 `400×38`，更新时效保留在无障碍描述和详情提示中 | 组件、样式与 Rust 尺寸测试 |
| Bar UI | Left/Right 侧轨 | Bar 随 1–7 个平台从 `64×156` 增至 `64×312`；Bottleneck 从 `64×100` 增至 `64×292`，所有文本保持正向，贴边侧平直、内侧圆角 | 组件、主题、方向与无障碍测试 |
...
- 自动发布门槛：发布助手通过 29 个文件中的 212 个前端测试、生产构建、72 个 Rust 测试、bundle budget、`fmt`、`check`、严格 `clippy`、diff 与版本一致性检查；正式发布前的独立 dry run 再次通过前端、构建和 Rust 测试。
- GitHub Actions：tag 触发的正式 `verify`、Windows Defender 实际产物扫描、Windows NSIS 发布、macOS Universal 发布、`finalize` 和 Windows `upgrade-smoke` 全部成功；已有 tag 的流程按预期跳过 `create-release-ref`。
- 公开 Release：`latest.json`、Windows 安装包及签名、macOS Universal DMG、macOS updater archive 及签名均已上传。
- 最新完整证据、链接、产物 SHA-256 与签名限制见 [RELEASE-0.3.7.md](RELEASE-0.3.7.md)。

这些结果证明构建、扫描、打包、更新签名和自动升级路径成功；它们不替代下列原生窗口视觉/交互矩阵。

## Windows 手动烟测

2026-09-12 的交互、设计与原生自动化补充验收见 [REVIEW-2026-09-12.md](REVIEW-2026-09-12.md)。
