...
4. React merges partial results with last-known-good values, derives pace/reset events, and persists bounded runtime history only when the persistable state changes. The latest 90 days retain full quota samples; older days retain their first, last, minimum, and maximum points through daily compaction. Daily usage summaries and coverage metadata remain local for the lifetime of the installation, subject to explicit safety caps.
