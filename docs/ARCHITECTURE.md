...
The Insights tab independently requests account aggregates through `get_codex_profile_stats`. Rust reads the same official `/wham/profiles/me` endpoint used by Codex Desktop Profile, selecting only six numeric statistics and a read timestamp. Credentials, account identity, and raw responses remain within Rust. Profile refreshes every ten minutes while mounted and supports manual refresh; local chart filters do not affect it. A native in-memory fingerprint tracks only successful login identities. Authentication failure or a changed identity invalidates the previous UI result, including overlapping failed requests; same-identity transient failure retains it with a stale notice. Account totals are not persisted or included in local exports. See [Profile data mapping and verification](CODEX-PROFILE-2026-09-17.md).

The Insights tab also lazily requests a separate Codex Token report covering retained active and archived session metadata files. Rust streams those files under explicit file, byte, line, and index-size safety caps, ignores message content, and persists a sanitized, versioned per-file cursor index in the application config directory. Stable rollout UUIDs deduplicate active/archive copies, preferring the more complete file; stored keys are SHA-256 hashes, not raw IDs or paths. A separate hashed source key prevents appending across changed physical sources. Unchanged files reuse indexed aggregates; append-only files resume from the saved byte cursor; truncation, source changes, metadata changes, or a manual rebuild reparses the affected scope. Read failures retain cached buckets with a partial indication. If a safety cap is reached, the report is explicitly marked partial. The UI receives only hourly numeric aggregates grouped by model, context tier, project basename, normalized terminal category, and a one-way hashed session key. This local range report is not the account Lifetime total.

Project identity uses a domain-separated SHA-256 digest of a lexically normalized path inside Rust; only an opaque ID and bounded basename label leave that boundary. Index schema 5 rebuilds older indexes from retained sources rather than guessing identity or reusing incompatible file keys. Project filters and export aliases use the ID, so equal basenames remain distinct. Raw paths are neither stored in the index nor sent to the WebView.
...
Failure is fail-closed: stale, malformed, oversized, redirected, timed-out, or unexpected-window responses are discarded independently. Only participating sources determine aggregate freshness. On a temporary collection failure, React retains a previously valid signal without extending its expiry and labels it cached. With no fresh result, the entry remains visible with an unavailable explanation, source links, and an independent public refresh action. Source conflicts suppress the combined estimate. Provider quota collection and personal planning remain independent, and an unhealthy account does not hide this public evidence entry in quota layouts or Insights.
