...
export function ControlCenter({ preferences, runtimeState, snapshots, diagnostics, language, onClose, onRefresh, onPreferences, onRuntimeState, onExport, onImport, onRestore, onCopyDiagnostics, autostartEnabled, onAutostart, operation = null }: Props) {
  ...
  const labels = zh ? {
    ...
    events: "最近事件", noEvents: "还没有额度变化事件", history: "历史采样", samples: "条采样", memoryTitle: "本地用量记忆", memoryEmpty: "首次刷新后开始记录", memoryRetention: "长期保留（受容量限制）· 90 天完整采样 · 更早记录按日压缩", lifetimeSamples: "累计采样",
    ...
    events: "Recent events", noEvents: "No quota change events yet", history: "History samples", samples: "samples", memoryTitle: "Local usage memory", memoryEmpty: "Starts after the first refresh", memoryRetention: "Capacity-limited retention · 90-day full detail · older samples compacted daily", lifetimeSamples: "Lifetime samples",
        ...
        <div className="control-view" key={tab}>
          ...
          <div className="provider-health-list">
            {PROVIDER_CATALOG.map((provider) => {
              ...
              const recovery = providerRecoveryAction(provider.id, status, language, diagnostics?.platform);
              return (
